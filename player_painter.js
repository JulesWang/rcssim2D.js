 ...
 */
function PlayerPainter(painter) {
...
PlayerPainter.prototype.draw = function(players) {

    for ( var i = 0; i < 1*2; ++i ) {
        this.drawPlayer( players[i] );
    }

    //if ( Options.showOffsideLine ) {
     //   this.drawOffsideLine( players );
    //}
}
 ...
 */
PlayerPainter.prototype.drawPlayer = function( player ) {

    this.drawBody( player );
    this.drawDir( player );
 ...
 */
PlayerPainter.prototype.drawBody = function( player ) {
    var fillColor;
    var strokeColor;
    ...
    case 'l':
        //if ( player.isGoalie() )
        {
            fillColor = LEFT_GOALIE_COLOR;
        }
        //else
        {
            fillColor = LEFT_TEAM_COLOR;
        }
        break;
    case 'r':
        //if ( player.isGoalie() )
        {
            fillColor = RIGHT_GOALIE_COLOR;
        }
        //else
        {
            fillColor = RIGHT_TEAM_COLOR;
        }
        break;
    case 'n':
   ...
   */
    ...
    this.painter.fillCircle( fillColor, player.x, player.y, PLAYER_RADIUS );
    // draw real body edge
    this.painter.drawCircle( strokeColor, player.x, player.y, PLAYER_RADIUS );
}

/*-------------------------------------------------------------------*/

PlayerPainter.prototype.drawDir = function( player ) {
    //const Options & opt = Options::instance();

    // body direction line
    var real_r = PLAYER_RADIUS + KICKABLE_MARGIN + BALL_RADIUS;
    var body = player.bodyDir;

    var bx = player.x + real_r * Math.cos( body );
    var by = player.y + real_r * Math.sin( body );

    this.painter.drawLine( PLAYER_PEN_COLOR, player.x, player.y, bx, by );
